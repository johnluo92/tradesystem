...
import MacroEconomic.ScraperManager;

// RealTimeBars Class is an implementation of the
// IB API EWrapper class
public final class Try_Me implements EWrapper {
    ...
    int transmit_buy, transmit_sell;
    double filledprice;
    boolean bullish;
    String direction;
    Thread t;
    ...
    public void OilStrategy(double inventory) {
        ...
        nextOrderID += 1;
        if (inventory < API) { // bullish story in which actual < API
...
// //        ScraperOil scraper = new ScraperOil(new TimeObject(15, 53), cb);
...
//        System.out.println("That took: " + (endTime-startTime) + "ms");
        ...
        switch (direction) {
            case "long":
                StopLoss.m_auxPrice = filledprice - .25;
                StopLoss.m_action = "SELL";
                ProfitTaking.m_lmtPrice = filledprice + .75;
